...
import com.video.view.VideoPlayerWindow;
...
/* TEST */   	System.out.println(UserController.getUserRep().getAllUsers().toString());
				sesionOpt = window.askStartSesion();
				if (sesionOpt == 0) {
...
/* TEST */ 					System.out.println(UserController.getUserRep().getAllUsers().toString());
			 ...
			 while (!end2) {
				//realiza opciones de video según corresponda
				switch (sesionOpt) {
				case 0:
					...
					break;
				case 1://Reproducir video
					String choosenVideoId = videoWindow.chooseVideo(userId);
					VideoPlayerWindow videoPlayer = null;
					if(choosenVideoId!=null){
							videoId = Integer.parseInt(choosenVideoId);	
